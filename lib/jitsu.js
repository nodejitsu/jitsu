...
//
// ### function gone
// Print a message that we're gone.
//
jitsu.gone = function () {
  jitsu.log.warn();
  jitsu.log.warn('Nodejitsu has been acquired by '.bold + 'GoDaddy'.green.bold);
  jitsu.log.warn();
  jitsu.log.warn('You have until '+ jitsu.gone.shutdown.red.bold +' to move, backup or migrate your applications');
  jitsu.log.warn('as this service will be shut down completely.');
  jitsu.log.warn();
};

//
// The date that our service will be fully shutdown.
//
jitsu.gone.shutdown = '01 August 2015';
...
jitsu.exec = function (command, callback) {
  function execCommand (err) {
    ...
    if (command[0] === 'signup' || command[0] === 'users' && command[1] === 'create') {
      jitsu.log.error();
      jitsu.log.error('New signup\'s has been disabled as the service will be shut down on: ');
      jitsu.log.error(jitsu.gone.shutdown.red.bold);
      jitsu.log.error();
