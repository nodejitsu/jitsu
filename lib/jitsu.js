...
jitsu.auth = function (callback) {
  ...
  winston.silly('Attempting to authenticate as ' + jitsu.config.username.grey);
  jitsu.users.auth(function (err, success) {
    if (err || !success) {
      winston.error('Unable to Autenticate as ' + jitsu.config.username.grey);
      winston.error(err.message);
      return callback(err);
    } 
    
    auth = true;
    winston.info('Authenticated as ' + jitsu.config.username.grey);
    callback();
  });
};
