        ...
        if(err.message && err.message === 'socket hang up'){
          if (err.code){
            if (err.code === 'ECONNRESET'){
              jitsu.log.info('\nThis type of error is usually an indication that the jitsu client\'s');
              jitsu.log.info('request timed out before the server could respond. If the application');
              jitsu.log.info('normally takes time to load, please wait a few minutes to see if the');
              jitsu.log.info('application deployed. Check the logs to see if there has been an error');
              jitsu.log.info('or logging output with `jitsu logs`.');
            }
          }
        } else {
