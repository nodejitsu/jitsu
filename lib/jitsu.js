...
jitsu.welcome = function () {
  ...
  winston.warn('This version of jitsu has been ' + 'deprecated'.bold);

  //
  // Check node.js version to find out if newer version of jitsu can be used.
  //
  var minor = process.version.split('.')[1];
  if (minor >= 6) {
    winston.info('Your version of node.js ' + 'supports'.green + ' newer versions of jitsu.');
    winston.info('You can upgrade by running:');
    winston.info('');
    winston.info('>'.grey + ' npm install jitsu -g');
    winston.info('');
  }
  else {
    winston.info('In order to use the latest version of jitsu, you will need');
    winston.info('to install a newer version of `node.js`.');
    winston.info('');
    winston.info('Unless you are running legacy code, we recommend upgrading');
    winston.info('to ' + 'node v0.6.x'.cyan + '.');
    winston.info('');
    winston.info('If you require assistance, please contact ' + 'support@nodejitsu.com'.cyan + '.');
    winston.info('');
  }
};
