...
jitsu.setup = function (callback) { 
  ...
  var options = {},
      port = jitsu.config.get('port');
  
  options.username = jitsu.config.get('username');
  options.password = jitsu.config.get('password');
  options.remoteUri = [
  ...
  jitsu.users = new jitsu.api.Users(options);
  jitsu.apps = new jitsu.api.Apps(options);
  jitsu.snapshots = new jitsu.api.Snapshots(options);
  jitsu.databases = new jitsu.api.Databases(options);
  jitsu.logs = new jitsu.api.Logs(options);
  jitsu.started = true;
