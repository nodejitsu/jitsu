...
jitsu.version       = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'package.json'))).version;

//
// Add the known properties to `jitsu.prompt` for later use
//
jitsu.prompt.properties = require('jitsu/properties').properties;

//
// Lazy load the REST clients for the nodejitsu APIs.
//
jitsu.api = {};
['Client', 'Apps', 'Snapshots', 'Users'].forEach(function (api) {
  jitsu.api.__defineGetter__(api, function () {
    return require('jitsu/api/' + api.toLowerCase())[api];
  });
});
...
jitsu.start = function (argv, callback) {
  ...
  var jitsuconf, localconf

  if(jitsuconf = argv.jitsuconf || argv.j){
    if(jitsuconf[0] != '/') //check it it's relative path
      jitsuconf = path.join(process.cwd(),jitsuconf)

    var dir = jitsuconf.split('/')
      , file = dir.pop()

    jitsu.config.settings.root = dir
    jitsu.config.settings.userconfig = file

  } else if(localconf = argv.localconf || argv.l){

    function recurse(dir){
      if(dir == process.env.HOME)
        return dir
      try {
        fs.statSync(path.join(dir,'.jitsuconf'))
        return dir
      } catch (err) {
        return recurse(path.join(dir,'..'))
      }
    }

    jitsu.config.settings.root = recurse(process.cwd())
  }
...
jitsu.setupUser = function (callback) {
  ...
  function getPassword (username) {
    jitsu.prompt.get(['password'], function (err, result) {
      ...
      jitsu.config.settings.auth = [username, result.password].join(':');
      jitsu.setup(function () {
        jitsu.auth(function (err) {
          ...
          if (err) {
            if (tries >= 3) {
              winston.error('Exiting. Three failed attempts as ' + username.magenta);
              return callback(err);
            }
  ...
  jitsu.prompt.get(['username'], function (err, result) {
    return err ? callback(err) : getPassword(result.username);
  });
};
