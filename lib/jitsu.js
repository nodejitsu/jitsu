  ...
  jitsu.config.load(function (err) {
    if (!jitsu.config.username) {
      return jitsu.setupUser(function (err) {
        ...
        if (command.length > 0) {
          winston.warn('Ignoring command ' + command.join(' ').yellow);
          winston.warn('Rerun this command now that your account is configured');
        }
...
jitsu.setupUser = function (callback) {
  ...
  jitsu.prompt.get(['username'], function (err, user) {
    jitsu.prompt.getHiddenInput('password', null, function (err, password) {
      jitsu.config.settings.auth = [user.username, password].join(':');
