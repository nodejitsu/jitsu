...
function tryAnalyze (target, dir, callback) {
  jitsu.apps.available(target, function (err, result) {
...
function fillPackage(base, dir, callback) {
  base = base || {};
  var toFill = ['name','subdomain','version'].filter(function(required,index) {
    if(base[required]) {
      return false;
    }
    return true;
  ...
  if(!(base.scripts && base.scripts.start)) {
    toFill.push('scripts.start');
  }
  var subdomain;
  var descriptors = package.properties(dir).filter(function(descriptor) {
    if(descriptor.name == 'subdomain') subdomain = descriptor;
    return toFill.indexOf(descriptor.name) !== -1;
  });
...
//
package.analyzeDependencies = function (pkg, dir, callback) {  //bug in this function. calls process.cwd() instead of the passed in dir.
  winston.info('Analyzing your application dependencies in ' + pkg.scripts.start.magenta);
  analyzer.analyze({ target: path.join(dir, pkg.scripts.start), timeout: 1000 }, function (err, pkgs) {
    //
