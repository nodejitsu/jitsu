...
package.createTarball = function (dir, version, callback) {
  ...
  package.read(dir, function (err, pkg) {
    ...
    var name = [jitsu.config.username, pkg.name, version || pkg.version].join('-') + '.tgz',
        tarball = path.join(jitsu.config.settings.tmproot, name);
    
    npm.load({ exit: false }, function () {
      npmtar.pack(tarball, dir, pkg, false, function (err) {
        return err ? callback(err) : callback(null, tarball);
      });
    });
  });
};
