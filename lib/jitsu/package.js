...
package.validate = function (pkg, dir, options, callback) {
  ...
  function checkProperty (desc, next) {
    ...
    invalid = !value || (desc.validator && !(
      desc.validator instanceof RegExp
        ? desc.validator.test(value)
  ...
  async.forEach(properties, checkProperty, function () {
    ...
    help = [
      '',
      'Your package.json file is missing and/or has invalid required fields:',
      '',
      names.join(', '),
      '',
...
package.updateTarball = function (version, pkg, existing, firstSnapshot, callback) {
  ...
  function executeCreate (err) {
    ...
    jitsu.package.createTarball(process.cwd(), version, function (err, ign, filename) {
      ...
      jitsu.log.warn('Creating new snapshot for version ' + version.magenta);
  ...
  if (!firstSnapshot) {
    ...
    if (old) {
      ...
      jitsu.log.warn('Your package.json version will be incremented for you automatically.');
