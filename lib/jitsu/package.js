  ...
  async.forEach(properties, checkProperty, function () {
    if (missing.length > 0) {
      ...
      return jitsu.prompt.addProperties(pkg, missing, function (err, updated) {
        return err ? callback(err) : package.checkDependencies(updated, dir, function (err, addedDeps) {
          return err ? callback(err) : package.write(addedDeps, dir, true, callback);
        })
      });
    }
    
    package.checkDependencies(pkg, dir, function (err, updated) {
      return err ? callback(err) : callback(null, updated);
    });
  });
};

package.checkDependencies = function (pkg, dir, callback) {
  ...
  analyzer.analyze({ target: path.join(process.cwd(), pkg.scripts.start), timeout: 1000 }, function (err, pkgs) {
    ...
    //
    var updated = analyzer.extractVersions(pkgs);
    
    if (package.newDependencies(pkg.dependencies, updated)) {
      winston.info('Found new dependencies');
      jitsu.log.putObject(updated);

      // 
      // Update the package.json dependencies
      //
      pkg.dependencies = analyzer.merge({}, pkg.dependencies || {}, updated);
    }
...
package.updateTarball = function (version, pkg, existing, callback) {
  jitsu.package.createTarball(process.cwd(), function (err, ign, filename) {
    if (err) {
    ...
    var old = semver.gte(existing.version, pkg.version),
        update = package.newDependencies(existing, pkg);
    ...
    if (old) {
      ...
      pkg.version = package.getVersion(existing.version);
    }
    ...
    if (old || update) {
      return package.write(pkg, process.cwd(), executeCreate);
    }
...
package.newDependencies = function (current, updated) {
  var updates = analyzer.updates(current.dependencies, updated.dependencies);
...
package.createTarball = function (dir, callback) {
  package.read(dir, function (err, pkg) {
    ...
    var parent = path.dirname(dir),
        ...
        tarball = path.join(jitsu.config.settings.tmproot, [jitsu.config.username, pkg.name, pkg.version].join('-') + '.tgz'),
