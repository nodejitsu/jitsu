...
utils.createPackage = function (dir, callback) {
  ...
  jitsu.prompt.get(utils.packageProperties(dir), function (err, result) {
    ...
    utils.writePackage(pkg, dir, true, callback);
  });
};
...
//
utils.writePackage = function (pkg, dir, create, callback) {
  if (!callback) {
    callback = create;
    create = null;
  }
  ...
  jitsu.prompt.get({
    ...
    if (result.answer !== 'yes' && result.answer !== 'y') {
      return create ? utils.createPackage(dir, callback) : callback(new Error('Save package.json cancelled.'));
    }
...
utils.versionPackage = function (pkg, version, dir, callback) {
  pkg.version = utils.getVersion(version);
  utils.writePackage(pkg, dir, callback);
};

utils.getVersion = function (version) {
  ...
  async.forEach(properties, checkProperty, function () {
    if (missing.length > 0) {
      ...
      return jitsu.prompt.addProperties(pkg, missing, function (err, updated) {
        return err ? callback(err) : utils.writePackage(updated, dir, true, callback);
      });
    }
