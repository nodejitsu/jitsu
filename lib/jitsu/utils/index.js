...
utils.createPackage = function (dir, callback) {
  var help = [
    '',
    'Lets walk through creating a new package.json file.',
    'We will only cover the most common items, and try to guess sane defaults.',
    'Press ^C at any time to quit.',
    ''
  ];
  
  winston.warn('There isn\'t a valid package.json file in ' + dir.magenta);
...
utils.writePackage = function (pkg, dir, callback) {
  winston.info('About to write ' + 'package.json'.magenta);
  jitsu.log.putObject(pkg, 2);
  jitsu.prompt.get({
    name: 'answer',
    message: 'Is this ' + 'ok?'.yellow.bold,
    default: 'yes'
  }, function (err, result) {
  ...
  async.forEach(properties, checkProperty, function () {
    if (missing.length > 0) {
      var help, names = missing.map(function (prop) {
        return (prop.message || prop.name).yellow;
      });
      ...
      help = [
        ...
        names.join(', '),
        'They need to be added before your app can be created.',
        'Press ^C at any time to quit.',
        ''
      ];
...
utils.packageProperties = function (dir) {
  return [
    ...
    },
    {
      ...
      validator: /[\w|\-|\_]+/,
      help: [
        'In order to create an application we\'ll need a unique subdomain'.cyan,
        'You\'re application will then become accessible at: http://yourdomain.nodejitsu.com'.cyan
      ]
    },
    {
      name: 'scripts.start',
