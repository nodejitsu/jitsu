...
users.create = function (username, callback) {
  ...
  function getPassword() {
    jitsu.prompt.get(['password', 'confirm password'], function (err, result) {
      if (result['password'] === result['confirm password'] && result['password'] !== '') {
        ...
        jitsu.log.info('You account is now being created');
        jitsu.users.create({
          username: username,
          email: email,
          password: password
        ...
        if (result['password'] === "") {
          jitsu.log.error('You password must have characters in it');
        } else {
          jitsu.log.error('The entered passwords do not match');
        }
  ...
  function getEmail() {
    jitsu.prompt.get('email', function (err, result) {
      ...
      email = result.email;
      jitsu.log.help('Finally, we will need a password for this account');
      getPassword();
    });
  }
    ...
    jitsu.prompt.get('username', function (err, result) {
      jitsu.users.available(result.username, function (err, res) {
        if (err) {
          ...
          jitsu.log.error('Username was already taken');
          return getUsername();
        }

        username = result.username;
        jitsu.log.help('Next, we will require your email address');
  ...
  jitsu.log.help('To signup, first you will need to provide a username');
...
users.available = function (username, callback) {
  ...
  jitsu.log.info('Checking availability for ' + username.magenta);
...
users.logout = function (callback) {
  ...
  jitsu.config.save(function (err) {
    ...
    jitsu.log.info('User has been logged out');
    callback();
  });
};
...
users.forgot = function (username, shake, callback) {
  ...
  callback = shake;
  jitsu.log.info('Request password reset for: ' + username.magenta);
  jitsu.users.forgot(username, {}, function (err, result) {
    if (err) {
      return callback(err);
    }

    jitsu.log.info('Check your email for instructions on resetting your password.');
    callback();
  });
};
...
users.whoami = function (callback) {
  ...
  jitsu.log.info('You are: ' + username.magenta);
};
  ...
  jitsu.prompt.get(['password', 'confirm password'], function (err, results) {
    ...
    if (results['password'] === results['confirm password'] && results['password'] !== "") {
      ...
      jitsu.users.forgot(username, {
        ...
        if (err) {
          jitsu.log.error('Unable to set new password for ' + username.magenta);
