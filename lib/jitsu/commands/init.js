...
var init = module.exports = function (starterName, callback) {
  ...
  function warnOverwrite(starterName, cb) {
    ...
    winston.warn('Copying a starter into this directory may overwrite other files.');
    jitsu.prompt.get(['overwrite'], function (err, results) {
      if (err) {
        return cb(err);
      }
      if (results['overwrite'] !== 'yes') {
        winston.info('Please switch to an empty directory to continue.');
        return cb(null);
      }
      return starterName ? createStarter({starter: starterName}, cb) : promptforStarterName(cb);
    });
  } 
  ...
  function createStarter(results, cb) {
    ...
    installStarter(starters[results['starter']], function (err, res) {
      ...
      return deployStarter(cb);
    });
  }

  function deployStarter(cb) { 
