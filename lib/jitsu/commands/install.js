...
module.exports = function (starterName, callback) {
  ...
  function postInstall(starterName, cb) {
    ...
    //
    app.config.schemaPath = './' + starterName + '/config/schema.json';
    path.exists(app.config.schemaPath, function (exists) {
      if (exists) {
        try {
          ...
          app.config.path = './' + starterName + '/config/development.json';
  ...
  function configureApp (schema, cb) {
    ...
    jitsu.log.help('Prompting for ' + starterName.magenta + ' configuration data');
    jitsu.log.help('You can hit [ENTER] to specify default values');
