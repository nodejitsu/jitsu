...
env.set = function (key, value, callback) {
  ...
  if (args.length !== 2) {
    jitsu.log.error('You must pass both <key> and <value>');
...
env.get = function (key, callback) {
  if (!callback) {
    callback = key;
    jitsu.log.error('No environment variables for ' + 'undefined'.yellow);
...
env.delete = function (key, callback) {
  if (!callback) {
    callback = key;
    jitsu.log.warn('No configuration for ' + 'undefined'.magenta);
    return callback();
  }

  var value = jitsu.config.get(key);
  if (!value) {
    jitsu.log.warn('No configuration value for ' + key.yellow);
    return callback();
  }
