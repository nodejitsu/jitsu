...
env.list = function (appName, callback) {
	var viewAppInfo = null;
	if(appName && callback) {
		viewAppInfo = function(viewByNameCallback, viewByNameSuccess) {
			viewAppByName(appName, viewByNameCallback, viewByNameSuccess)
		}
	} else {
		viewAppInfo = viewApp
		callback = appName
	}
  viewAppInfo(callback, function (err, app) {
...
function viewAppByName (appName, callback, success) {
	appName = appName.search('/') === -1 ?  jitsu.config.get('username') + '/' + appName : appName;
