  ...
  jitsu.apps.list(username, function (err, apps) {
    ...
    apps.forEach(function (app) {
      if(app.state === "started") {
        app.state = app.state.green;
      } else {
        app.state = (app.state || 'unknown').grey;
      }
      ...
      rows.push([
        ...
        app.state,
        app.subdomain,
        app.snapshots && app.snapshots.length > 0 ? app.active.filename : '---'
      ]);
    });
...
apps.view = function (name, callback) {
  ...
  function getAppName(callback) {
    jitsu.package.read(process.cwd(), function (err, pkg) {
      if (!err) {
        jitsu.log.info('Attempting to load application: ' + (process.cwd()+ '/package.json').grey);
