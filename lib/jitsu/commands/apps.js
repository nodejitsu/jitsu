...
apps.deploy = function (callback) {
  jitsu.utils.readPackage(process.cwd(), function (err, pkg) {
    ...
    if (err) {
      winston.error('No package.json file found.');
    ...
    function checkSnapshots (err, existing) {
      ...
      winston.warn('Need name for new snapshot');
      jitsu.commands.commands.snapshots.create(existing.name, function (err, snapshot) {
        ...
        jitsu.commands.commands.snapshots.activate(existing.name, snapshot, function (err) {
...
apps.list = function (callback) {
  ...
  jitsu.apps.list(function (err, apps) {
    ...
    if (!apps || apps.length === 0) {
      ...
      winston.info('Try creating one with ' + 'jitsu apps create'.magenta);
      return callback();
    }
...
apps.start = function (name, callback) {  
  function executeStart() {
    ...
    jitsu.apps.start(name, function (err) {
      winston.info('App ' + name.magenta + ' is now started at: ' + app.subdomain + ':' + app.port);
      return err ? callback(err) : callback();
    });
  }
