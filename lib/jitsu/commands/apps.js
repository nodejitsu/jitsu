...
apps.deploy = function (callback) {
  ...
  function updateSnapshot (err, existing) {
    if (err) {
      winston.error('Error creating app ' + pkg.name.magenta + ': ' + err.message.red);
      return callback(new Error(), true);
    }
    
    jitsu.package.updateTarball(null, pkg, existing, function (err, snapshot) {
      if (err) {
  ...
  function checkApp (err, local, before) {
    if (err) {
      winston.error(err);
      return callback(new Error(), true);
    }
    ...
    jitsu.apps.view(local.name, function (err, app) {
      if (err) {
        ...
        //
        return !before ? apps.create(local, updateSnapshot) : before(app, function (err, updated) {
          if (err) {
            return callback(err);
          }
          
          pkg = updated;
          apps.create(updated, updateSnapshot);
        });
      }
      
      return !before ? updateSnapshot(null, app) : before(app, function (err, updated) {
        pkg = updated;
        return err ? updateSnapshot(err) : updateSnapshot(null, updated);
      })
    });
  }
  ...
  jitsu.package.read(dir, function (err, local) {
    ...
    checkApp(null, local, function (app, done) {
      app = jitsu.utils.formatApp(app, ['snapshots', 'active', 'user', 'state'])
      var pkg = analyzer.merge({}, local, app);
      jitsu.package.validate(pkg, dir, done);  
    });
  });
};
