  ...
  function listObject(obj, prefix) {
    ...
    Object.keys(obj).sort().forEach(function (key) {
      if (Array.isArray(obj[key])) {
        winston.data((prefix + key) + ' ' + obj[key].join(' '));
      }
      else {
        if ( key == "auth" ) {
          obj[key] = obj[key].split(':')[0] + ':******';
        }
        winston.data('  "' + (prefix + key) + '": ' + '"' + obj[key].toString() + '"');
      }
    });
  }
  
  winston.info('Hello '.cyan + jitsu.config.settings.auth.split(':')[0].green + ' here is your ' + jitsu.config.settings.root.magenta + '/.jitsuconf'.magenta + ' file:');
  winston.info('');
  winston.data('{');
  listObject(jitsu.config.settings);
  winston.data('}');
  winston.info('');
  winston.info('If you\'d like to change a property try:'.cyan);
  winston.info('');
  winston.info('  jitsu config set <key> <value>'.cyan);
  winston.info('');
  winston.info('or...'.cyan);
  winston.info('');
  winston.info('  jitsu config help'.cyan);
  winston.info('');
  callback();
};
